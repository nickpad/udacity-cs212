...
def subway(**lines):
    ...
    for (line, stations) in lines.items():
        slist = stations.split(' ')
        pstation = None

        for (i, station) in enumerate(slist):
            try:
                nstation = slist[i + 1]
            except IndexError:
                nstation = None

            if station not in map:
                map[station] = {}

            for neighbour in (pstation, nstation):
                if neighbour:
                    map[station][neighbour] = line
...
def longest_ride(system):
    ...
    trips = itertools.combinations(system.keys(), 2)
    paths = [ride(x, y, system) for (x, y) in trips]
