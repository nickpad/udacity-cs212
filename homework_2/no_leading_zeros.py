...
def compile_formula(formula, verbose=False):
    ...
    predicate_terms = [('%s != 0' % w[0]) for w in words if w.isupper()]
    predicate = ' and '.join(predicate_terms)
    tokens = map(compile_word, words)
    body = ''.join(tokens)
    f = 'lambda %s: %s and %s' % (parms, predicate, body)
    if verbose: print(f)
    return eval(f), bytes(letters, 'ascii')
...
def faster_solve(formula):
    ...
    for digits in itertools.permutations((1,2,3,4,5,6,7,8,9,0), len(letters)):
        try:
            if f(*digits) is True:
                table = bytes.maketrans(letters, bytes(''.join(str(d) for d in digits), 'ascii'))
